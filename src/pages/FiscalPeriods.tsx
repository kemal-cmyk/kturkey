...
export default function FiscalPeriods() {
  ...
  return (
    <div className="p-6 space-y-6">
            ...
            <div className="space-y-2">
              {periods.map((period) => (
                ...
                >
                  ...
                  <p className="text-sm font-medium text-[#002561] mt-1">
        ...
        <div className="lg:col-span-2">
          {selectedPeriod ? (
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-gray-500">
                      {format(new Date(selectedPeriod.start_date), 'MMMM d, yyyy')} -{' '}
                      {format(new Date(selectedPeriod.end_date), 'MMMM d, yyyy')}
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gray-50 rounded-xl p-4">
                    ...
                    <p className="text-2xl font-bold text-gray-900">
                      {formatCurrency(selectedPeriod.total_budget)}
                    </p>
                  </div>
                  <div className="bg-gray-50 rounded-xl p-4">
                    <p className="text-sm text-gray-500">Monthly Average</p>
                    <p className="text-2xl font-bold text-gray-900">
                      {formatCurrency(selectedPeriod.total_budget / 12)}
                    </p>
                  </div>
                </div>
                            ...
                            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full transition-all ${
                                  utilization > 100 ? 'bg-red-500' : 'bg-[#002561]'
                                }`}
                ...
                {isAdmin && selectedPeriod.status === 'active' && (
                  <div className="bg-orange-50 border border-orange-200 rounded-xl p-4">
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        <p className="font-medium text-orange-900">Year-End Closing</p>
                        <p className="text-sm text-orange-700 mt-1">
                          When this period ends, you can perform a rollover to transfer outstanding
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
              ...
              <p className="text-gray-500">Select a financial period to view details</p>
            </div>
          )}
        </div>
      </div>
      ...
      {showAddCategoryModal && selectedPeriod && (
        <AddCategoryModal
      ...
      {editingCategory && selectedPeriod && (
        <EditCategoryModal
      ...
      {addingEntryCategory && selectedPeriod && currentSite && user && (
        <AddEntryModal
          category={addingEntryCategory}
          period={selectedPeriod}
          siteId={currentSite.id}
          userId={user.id}
          accounts={accounts}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-100">
          <h3 className="text-xl font-semibold text-gray-900">Create Financial Period</h3>
        </div>
...
function RolloverModal({ period, siteId, onClose, onCompleted }: RolloverModalProps) {
  ...
  const handleRollover = async () => {
    ...
    await supabase.rpc('perform_fiscal_year_rollover', {
      p_closing_period_id: period.id,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full">
        <div className="p-6 border-b border-gray-100">
          <h3 className="text-xl font-semibold text-gray-900">Year-End Rollover</h3>
          <p className="text-gray-500 mt-1">Transfer balances to the next financial period</p>
        </div>
          ...
          <div className="bg-orange-50 border border-orange-200 rounded-xl p-4">
            <div className="flex items-start space-x-3">
              ...
              <div className="text-sm text-orange-700">
                <p className="font-medium text-orange-900 mb-1">This action will:</p>
                <ul className="list-disc list-inside space-y-1">
                  <li>Close the current period: {period.name}</li>
                  <li>Transfer outstanding debts to the new period</li>
                  <li>Preserve legal action statuses (Icra)</li>
                  <li>Create "Previous Period Balance" entries</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            {availablePeriods.length > 0 ? (
              ...
              <p className="text-gray-500 text-sm">
                No draft periods available. Please create a new financial period first.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="p-6 border-b border-gray-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              Category: <span className="font-medium text-gray-700">{category.category_name}</span>
            </p>
          </div>
          <button
            type="button"
            ...
            >
              ...
              {accounts.map(acc => (
                <option key={acc.id} value={acc.id}>
                  {acc.account_type === 'bank' ? 'Bank' : 'Cash'}: {acc.account_name} ({acc.currency_code})
                </option>
              ))}
            </select>
            {accounts.length === 0 && (
              <p className="text-xs text-amber-600 mt-1">
                No accounts configured. Please add an account in the Ledger first.
              </p>
            )}
          </div>
...
function SetDuesModal({ periodId, siteId, defaultCurrency, onClose, onSet }: SetDuesModalProps) {
  ...
  const handleSetUniform = async () => {
    ...
    const { error } = await supabase.rpc('set_all_units_monthly_due', {
      p_fiscal_period_id: periodId,
      p_monthly_amount: Number(uniformAmount),
  ...
  const handleSetIndividual = async () => {
    const unitData = Object.entries(unitAmounts)
    ...
    const { error } = await supabase.rpc('set_varied_unit_monthly_dues', {
      p_fiscal_period_id: periodId,
      p_unit_amounts: unitData,
      p_currency_code: currency,
    });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        <div className="p-6 border-b border-gray-100">
          <h3 className="text-xl font-semibold text-gray-900">Set Monthly Dues</h3>
          <p className="text-sm text-gray-500 mt-1">
            Configure the monthly due amounts for units in this fiscal period
          </p>
        </div>
        ...
        <div className="p-6 space-y-6 overflow-y-auto flex-1">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Monthly dues will be recorded in this currency
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <div className="text-xs mt-1">Set different amounts per unit</div>
            </button>
          </div>
          ...
          {mode === 'uniform' ? (
            <div>
              ...
              <p className="text-sm text-gray-500 mt-2">
                This amount will be set for all {units.length} units
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              <label className="block text-sm font-medium text-gray-700">
                Set amounts for each unit
              </label>
              <div className="max-h-96 overflow-y-auto space-y-2 border border-gray-200 rounded-lg p-3">
                {units.map((unit) => (
                  <div key={unit.id} className="flex items-center space-x-3 p-2 bg-gray-50 rounded-lg">
                    <div className="flex-1">
                      <div className="font-medium text-gray-900">Unit {unit.unit_number}</div>
                      {unit.owner_name && (
                        <div className="text-xs text-gray-500">{unit.owner_name}</div>
                      )}
                    </div>
                    <input
                      type="number"
                      ...
                      onChange={(e) =>
                        setUnitAmounts({ ...unitAmounts, [unit.id]: e.target.value })
                      }
