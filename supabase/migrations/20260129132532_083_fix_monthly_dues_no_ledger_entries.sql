/*
  # Fix Monthly Dues Functions - Remove Ledger Entry Creation
  
  ## Problem
  The set_unit_monthly_due function was calling apply_unit_payment() to re-apply
  existing payments, which created duplicate ledger entries. This caused the Ledger
  page to show duplicate records even on empty ledgers.
  
  ## Solution
  Modify set_unit_monthly_due to:
  1. Store existing payment data
  2. Delete old dues and payments
  3. Create new dues records
  4. Re-insert payment records WITHOUT triggering ledger entry creation
  5. Manually update dues with paid amounts based on re-inserted payments
  
  ## Key Changes
  - Remove apply_unit_payment() call (which creates ledger entries)
  - Directly insert into payments table to restore payment records
  - Manually update dues paid_amount and status based on payments
  - Dues functions remain completely disconnected from ledger_entries table
  
  ## Result
  The dues table is recalculated without creating new ledger entries.
  Existing payment records are restored without duplicating ledger activity.
*/
...
BEGIN
  ...
  -- Re-insert all existing payments WITHOUT creating ledger entries
  -- This preserves payment history without duplicating ledger activity
  IF v_payment_amounts IS NOT NULL THEN
    FOR v_idx IN 1..array_length(v_payment_amounts, 1)
    LOOP
      -- Re-insert payment record
      ...
      -- Now manually apply this payment to dues without creating ledger entries
      -- This mirrors the logic from apply_unit_payment but without the ledger insert
